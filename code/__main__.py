...
if 'other' in args and args['other'] == "evoplot":
    plot_evolver_data('plans/' + args['f'])

elif 'algo' in args:
    visualize = parseBool(args['vis'])

else: raise Exception("Need either algo=??? other other=???")

if args['algo'] in algos and args['algo'] != 'ex':

    def parseBase(b, enable_playground, num_houses):
        if b == "dynamic":
            from src.base_dynamic import base_dynamic as d
        elif b == "a":
            from src.base_a import base_a as d
        elif b == "b":
            from src.base_b import base_b as d
        elif b == "c":
            from src.base_c import base_c as d
        else: raise Exception("Unrecognized base keyword")
...
if args['algo'] == 'ts':
    ...
    def parse_initState(s, b):
        if s == "rndm": return validstate_rndm(b).plan
        elif s == "tight": return validstate_tight(b).plan
