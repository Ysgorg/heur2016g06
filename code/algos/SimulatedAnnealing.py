...
def get_acceptance_probability(
        current_value, new_value, temperature, max_temperature):
    if temperature == 0:
        return 0.0
    else:
...
def simulated_annealing(
        init_state, max_iterations, gen_neigbor_func, frame=None):
    ...
    for i in range(max_iterations):
        ...
        neighbor = gen_neigbor_func(
            state.deepCopy(), temperature).deepCopy()

        # while True: pass
        # print "i =",i
        # If the new plan has a lower value, calculate the acceptance threshold
        # of still accepting this state (probability decreases as temperature
        # does)
        if neighbor.getPlanValue() < state.getPlanValue():
            ...
            if accept_probability > random_val:
                ...
                jump_count += 1
                # print "Accepted new state with probability: ",
                # accept_probability, ">", random_val
        ...
        if state.getPlanValue() > best_state.getPlanValue():
            ...
            if (1.0 - prev_best.getPlanValue() / best_state.getPlanValue()) * 100 <= MIN_PERCENTAGE_CHANGE:
                # print "Change between previous best and current best less
                # than", MIN_PERCENTAGE_CHANGE, "% Terminating search."
                break
