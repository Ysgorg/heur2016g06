    ...
    for nh in variables[0]:
        for pg in variables[1]:
            for base in variables[2]:
                if base == base_c.base_c and pg:
                    continue
                b = base(
                    num_houses=nh, enable_playground=pg).develop_ground_plan()
                for init_s in variables[3]:
                    # print nh,pg,base,init_s
                    if init_s == TightFit_A.validstate_tight:
                        ins = init_s(
                            b.deepCopy(), 1.0, 1.0, 1.0).getPlan().deepCopy()
                    else:
                        try:
                    ...
                    for ng in variables[4]:
                        if ng == neighbor_tight.neighbor_tight and not init_s == TightFit_A.validstate_tight:
                            continue
                        r = simulated_annealing(
                            ins.deepCopy(), MAX_ITERATIONS, ng)
                        frame.repaint(r)
...
def report(frame):
    experiment_variables = [
        ...
        [base_dynamic.base_dynamic         # , base_a.base_a
