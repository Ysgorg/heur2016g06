...
def perform_experiments(nh, pg, bases, init_states, neigbor_functions, max_iterations, frame):
    ...
    for base in bases:
        ...
        for init_s in init_states:
            if init_s == TightFit_A.TightFit_A: ins = init_s(b.deepCopy(), 1.0, 1.0, 1.0).getPlan().deepCopy()
            else:
                try:ins = init_s(b.deepCopy()).getPlan().deepCopy()
...
def report(num_houses, enable_pg, settings, frame):
