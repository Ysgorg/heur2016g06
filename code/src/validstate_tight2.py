...
class validstate_tight2(object):
    ...
    def coords(self,plan,rt,factor,t):
        ...
        r = rt(0,0)
        clearance = r.minimumClearance*factor
        h = r.height
        w = r.width
        ...
        while placed_houses<goal:
            ...
            y=-1
            x=x_init
            cur_x = self.translate(plan,clearance,h,w,x-1,0,t)[0]
            if cur_x > plan.WIDTH or cur_x < 0:
                return plan
            while x>0 and placed_houses<goal:
                x-=1
                y+=1
                res = self.translate(plan,clearance,h,w,x,y,t)
                #if res[0]>plan.WIDTH or res[1]>plan.HEIGHT:break
