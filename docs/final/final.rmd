...
The 70-residence problem instance is an exception which we can't explain. 
...
By comparing our different algorithm components statistically, we are able to say some generally outperform others. 
Zoom is preferable over SA, Tight Fit A outperforms Tight Fit B which outperforms HillClimber. Out HillClimber implementation doesn't scale.  
The bases we tested had little but noticable difference. Base A is generally preferable. 
...
```{r, out.width = "220px", echo=FALSE}
knitr::include_graphics("manual_images/advanced.png")
```
